...
fn solve(input: &mut Input, _test_case: usize) {
    let _: i64 = input.read();
    let a: i64 = input.read();
    let b: i64 = input.read();
    let p: i64 = input.read();
    let q: i64 = input.read();
    let r: i64 = input.read();
    let s: i64 = input.read();

    let ans = Arr2d::generate((q - p + 1) as usize, (s - r + 1) as usize, |row, col| {
        if ((row as i64) + p - a).abs() == ((col as i64) + r - b).abs() {
            '#'
        } else {
            '.'
        }
...
fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./c_x_drawing/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
