...
fn solve(input: &mut Input, _test_case: usize) {
    let mut x1: u32 = input.read();
    let mut p1: u32 = input.read();
    let mut x2: u32 = input.read();
    let mut p2: u32 = input.read();

    while x1 < 100000 && p1 > 0 {
        x1 *= 10;
        p1 -= 1;
    }
    while x2 < 100000 && p2 > 0 {
        x2 *= 10;
        p2 -= 1;
    }
...
fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./a_dlinnoe_sravnenie/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
