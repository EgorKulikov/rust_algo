...
fn solve(input: &mut Input, _test_case: usize) {
    let n = input.read();
    let mut s = input.read_vec::<i32>(n);
    s.sort();
    let low: i32 = s[..n / 2].iter().sum();
    let high: i32 = s[n / 2..].iter().sum();
    let win = 1f64 / (1f64 + 10f64.powf(((low - high) as f64) / 400f64));
    out_line!(high, win);
}
...
fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./d_tug_of_war/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
