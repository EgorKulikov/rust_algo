...
fn solve(input: &mut Input, _test_case: usize) {
    ...
    let mut t = input.read_vec::<u32>(m);
    let p = input.read_vec::<u32>(n);
    ...
    for (p, res) in p.iter().zip(ans.iter_mut()) {
        *res = if p * total_time >= k { 1 } else { 0 };
    }
    out_line!(ans);
}
...
fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./c_red_light_green_light/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
