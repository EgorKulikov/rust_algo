//{"name":"witch_s_cauldron_part_3","group":"Manual","url":"","interactive":false,"timeLimit":2000,"tests":[{"input":"","output":""}],"testType":"multiNumber","input":{"type":"stdin","fileName":null,"pattern":null},"output":{"type":"stdout","fileName":null,"pattern":null}}
...
fn solve(input: &mut Input, out: &mut Output, test_case: usize, _data: &mut PreCalc) {
    ...
    let toxic = input.read_line_vec(k);
    ...
    for mask in usize::iter_all(k) {
        ...
        let mut res = mem.call(Str::from(b"witch's brew"));
        if res != -1 {
            for i in 0..k {
                if mask.is_set(i) && toxic[i] != Str::from(b"witch's brew") {
                    res += c;
                }
            }
            ans.minim(res);
