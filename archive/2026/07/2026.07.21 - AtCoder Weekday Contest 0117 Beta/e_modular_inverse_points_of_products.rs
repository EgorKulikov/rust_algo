...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, _data: &mut PreCalc) {
    ...
    let b = Vec::with_gen(n, |i| Mod::from(a[i]).inv().unwrap_or(Mod::zero()));
    let mut mem = Memoization2d::new(n + 1, k + 1, |mem, pos, rem| -> Mod {
        if pos == n {
            if rem == 0 { Mod::one() } else { Mod::zero() }
        } else {
            let mut res = mem.call(pos + 1, rem);
            if rem > 0 {
                res += mem.call(pos + 1, rem - 1) * b[pos];
            }
            res
        }
    });
