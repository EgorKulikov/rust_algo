...
def ears(cy,cx,r,t,fill=False):
    ...
    for side in (-1,1):
        ...
        if fill: seg=rr<=r
        else: seg=(rr>=r-t)&(rr<=r)
        seg &= (xx>=cxs) if side>0 else (xx<=cxs)   # outer half (C opens toward bowl)
        m|=seg
    return m
def avg(img,k=8,tag=''): return float(np.mean([b.query(img,tag=tag)[0] for _ in range(k)]))
print('base:',round(avg(base,10,'ae_b'),3),flush=True)
best=(-1,None,'')
for cx in (176,179,182):
  for cy in (98,101,104):
    for r in (7,9,11,13):
      for t in (3,4):
        h=ears(cy,cx,r,t)
        sc=avg(base|h,8,f'ae{cx}_{cy}_{r}_{t}')
