...
for p in sel:
    ...
    if k < len(idxs):
        ...
        used[p] = k + 1
import numpy as _np
_rng = _np.random.default_rng(777)
# random-restart: keep 60% of current selection, fill rest by colored-score order with noise
_all = list(range(N))
_keep = [i for i in cur_set if _rng.random() < 0.6]
cur_set = set(_keep)
_rest = [i for i in _all if i not in cur_set]
_rng.shuffle(_rest)
for _i in _rest[: 999 - len(cur_set)]:
    cur_set.add(_i)
print(f"start selection (seed 777): {len(cur_set)}")
...
with open("out/03s777.out", "w") as f:
