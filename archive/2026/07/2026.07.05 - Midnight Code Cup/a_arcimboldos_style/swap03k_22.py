...
for p in sel:
    ...
    if k < len(idxs):
        ...
        used[p] = k + 1
import numpy as _np
_rng = _np.random.default_rng(22)
_kept = sorted(cur_set)
_drop = _rng.choice(_kept, size=50, replace=False)
for _i in _drop: cur_set.discard(int(_i))
_rest = [i for i in range(N) if i not in cur_set]
_add = _rng.choice(_rest, size=50, replace=False)
for _i in _add: cur_set.add(int(_i))
print(f"start selection (kick seed 22): {len(cur_set)}")
...
with open("out/03k22.out", "w") as f:
