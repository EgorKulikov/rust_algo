...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, _data: &mut PreCalc) {
    ...
    let pos = by_index(&a);
    type Mod = ModIntF;
    let mut mem = Memoization2d::new(n, n, |mem, x, y| -> (Mod, Mod) {
        let mut cur = Mod::from(1);
        let p1 = pos[a[x] + a[y]].lower_bound(&y);
        if p1 != pos[a[x] + a[y]].len() {
            cur += mem.call(y, pos[a[x] + a[y]][p1]).1;
        }
        let p2 = pos[a[y]].upper_bound(&y);
        let mut sum = cur;
        if p2 != pos[a[y]].len() {
            sum += mem.call(x, pos[a[y]][p2]).1;
        }
        (cur, sum)
    });
