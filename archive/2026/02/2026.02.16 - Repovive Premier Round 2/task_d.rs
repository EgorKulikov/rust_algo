...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, _data: &mut PreCalc) {
    ...
    let num_kings = s.copy_count(b'K');
    let mut at = 0;
    let mut table = Arr2d::new(n, num_kings, 0i64);
    for (r, c) in s.indices() {
        if s[(r, c)] == b'K' {
            for i in 0..n {
                table[(i, at)] = r.abs_diff(i).max(c.abs_diff(i)) as i64;
            }
            at += 1;
        }
    }
    out.print_line(hungarian_algorithm(&table));
}
