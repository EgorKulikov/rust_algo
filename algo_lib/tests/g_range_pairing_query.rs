...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, _data: &mut PreCalc) {
    ...
    let qq = input.read_size_pair_vec(q).dec();

    struct Node {
        ans: usize,
        qty: DefaultHashMap<usize, usize>,
    }
    impl MoWorker for Node {
        type T = usize;
        type R = usize;
        ...
        fn remove_left(&mut self, val: &usize) {
            if self.qty[*val] % 2 == 0 {
                self.ans -= 1;
            }
            self.qty[*val] -= 1;
...
pub(crate) fn run_tests() -> bool {
    let path = "./g_range_pairing_query";
    let tl = 5000;
