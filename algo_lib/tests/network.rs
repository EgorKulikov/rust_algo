//{"name":"1160. Network","group":"Timus","url":"https://acm.timus.ru/problem.aspx?space=1&num=1160","interactive":false,"timeLimit":1000,"tests":[{"input":"4 6\n1 2 1\n1 3 1\n1 4 2\n2 3 1\n3 4 1\n2 4 1\n","output":"1\n4\n1 2\n1 3\n2 3\n3 4\n"}],"testType":"single","input":{"type":"stdin","fileName":null,"pattern":null},"output":{"type":"stdout","fileName":null,"pattern":null},"languages":{"java":{"taskClass":"Network"}}}
...
fn solve(input: &mut Input, _test_case: usize) {
    ...
    for (f, t, w) in edges {
        graph.add_edge(f - 1, BiWeightedEdge::new(t - 1, w));
    }
    let tree = graph.minimal_spanning_tree();
    let mut max_weight = None;
    for i in 0..n {
        for e in &tree[i] {
            max_weight.maxim(e.weight());
        }
    }
    out_line!(max_weight);
    assert!(tree.is_tree());
    out_line!(tree.edge_count());
    for i in 0..n {
        for e in &tree[i] {
            if i < e.to() {
                out_line!(i + 1, e.to() + 1);
...
pub(crate) fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./tests/network/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
