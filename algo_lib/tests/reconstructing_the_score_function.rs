...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, _data: &mut PreCalc) {
    ...
    let x = input.read_vec::<Mod>(n);
    let y = input.read_vec::<Mod>(n);
    let z = input.read_vec::<Mod>(m);

    let interpolation = Interpolation::from_points(&x.copy_zip(&y).collect::<Vec<_>>());
    out.print_line(interpolation.calculate_many(&z));
}
...
pub(crate) fn run_tests() -> bool {
    let path = "./reconstructing_the_score_function";
    let tl = 2000;
...
#[test]
fn reconstructing_the_score_function() {
